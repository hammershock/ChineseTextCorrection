...
def generate_difference_mask(original: Sequence[_T], corrected: Sequence[_T], null_fill: _T) -> Tuple[
    List[int], List[_T]]:
    """
    比较两个序列的不同, 生成一个与original序列相同形状的mask序列, 0的位置代表正确, 1的位置代表错误
    >>> original_texts = "你好玛，你的名子角什么牙,,"
    >>> correct_texts = "你好啊, 你的名字叫什么呢"
    >>> mask, corrected_original = generate_difference_mask(original_texts, correct_texts, '[BLANK]')
    >>> mask
    [0, 0, 1, 1, 0, 0, 0, 1, 1, 0, 0, 1, 0, 0]
    >>> corrected_original
    ['你', '好', '啊', ',', '你', '的', '名', '字', '叫', '什', '么', '呢', '[BLANK]', '[BLANK]']
    """
...
def pinyin_tokenize(text: str, tokenizer: BertTokenizer, pad_value="[OTHER]") -> Tuple[np.array, np.array]:
    ...
    tokens = np.array(tokenizer.tokenize(text), dtype="<U10")  # pinyin和token一般不会超出U10
...
def _generate_data_collate(text_lines: List[str], correct_lines: List[str],
                           tokenizer: BertTokenizer, pinyin_vocab, max_len=128, overlap=64) -> Iterator[
    Tuple[np.ndarray, ...]]:
...
def _make_dataset(original_lines, correct_lines, tokenizer, pinyin_vocab, max_len=128, overlap=64):
    token_ids, corrected_ids, py_token_ids, correct_mask, attn_mask = zip(
        *_generate_data_collate(original_lines, correct_lines, tokenizer, pinyin_vocab, max_len=max_len,
                                overlap=overlap))
